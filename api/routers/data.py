...
def at_risk(
    ...
    df    = store.get_at_risk(threshold=threshold, classes=_parse_classes(classes))
...
def stats(
    ...
    store = get_store(request)
    df    = store.compute_stats(group_by=group_by, period=period, classes=_parse_classes(classes))
    return [] if df.empty else df.to_dict(orient="records")
